...
// Sends 5 TRX so user can approve the GasStation contract.
// This is the ONLY time the relayer sends TRX to a user.
...
app.post('/api/fund-for-approval', async (c) => {
  try {
    ...
    // Already funded? Don't send more.
    ...
    // Check if user already has enough TRX (e.g. from another source)
    ...
    if (currentBalance >= 3) {
      // User already has enough TRX, just record it without sending
      recordFunding(address, 0, 'already-has-trx');
      return c.json({
        success: true,
        alreadyFunded: true,
        message: `User already has ${currentBalance.toFixed(1)} TRX. Proceed with approval.`,
      });
    }
